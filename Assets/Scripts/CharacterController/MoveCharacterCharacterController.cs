...
public class MoveCharacterCharacterController : MonoBehaviour {
	...
	private bool m_shouldSlide;
	private bool m_sliding;

	private bool m_shouldHoldOntoLedge;
	...
	private void GetInput()
	{
		...
		if(Mathf.Abs(m_verticalInput) < 0.1f)
		{
			//m_verticalInput = 0;
		}
	...
	private void CheckEnvironment()
	{
		...
		if(m_characterController.isGrounded && m_inAir)
		{
			if(Vector3.Angle(m_frontHit.normal, Vector3.up) > m_characterController.slopeLimit)
			{			
				m_shouldSlide = true;
			}else{
				m_shouldSlide = false;
	...
	private void ExecuteMovement()
	{
		...
		if(m_isGrounded && !m_shouldSlide)m_moveDirection[1] = 0;
	...
	private void ExecuteAnimations()
	{
		...
		/*
		m_animator.SetFloat("LeftFoot", m_combinedSpeed > 0.1f ? 1 : 0);
		m_animator.SetFloat("RightFoot", m_combinedSpeed > 0.1f ? 1 : 0);
		*/
